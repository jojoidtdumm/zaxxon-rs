...
async fn main() {
    ...
    loop {
        ...
        if is_key_down(KeyCode::W) && player_altitude < MAX_ALTITUDE {
            player_altitude += 0.25;
            vel2_y += PLAYER_SPEED;
        }

        if is_key_down(KeyCode::S) && player_altitude > 0.0 {
            player_altitude -= 0.25;
            vel2_y -= PLAYER_SPEED;
        }
        ...
        player_rect.x += vel_x;
        player_rect.y += vel_y;
        player_rect.y -= vel2_y;
        //player_rect.y -= player_altitude;
        //println!("{}, {}", player_altitude, vel_x);
        player_altitude = player_altitude.clamp(0.0, 100.0);
